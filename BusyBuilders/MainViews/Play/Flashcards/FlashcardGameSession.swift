                    ...
                    Button("Done"){
                        if let user = users.first {
                            if !isPracticeSession {
                                if gameFlashcards.count == 1{
                                    let newSession = FlashcardSessionDataModel(sessionDate: sessionDate, sessionWin: sessionWin, sessionScore: sessionScore, sessionValue: sessionValue, sessionType: .singleDeck)
                                    user.flashcardSessions.append(newSession)
                                }
                                else {
                                    let newSession = FlashcardSessionDataModel(sessionDate: sessionDate, sessionWin: sessionWin, sessionScore: sessionScore, sessionValue: sessionValue, sessionType: .multiDeck)
                                    user.flashcardSessions.append(newSession)
                                }
                            }
                            ...
                            do {
                                ...
                                print("Couldnt Save the flashcard Session")
                            }
                        }
